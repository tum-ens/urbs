...
def extract_number_str(str_in):
    """ Extract the first number from a given string and
        ...
        Excel sheet: Commodity - price - for Buy and Sell:

            - fix price: (0.12) [input type: float]
                the same price for each timestep
            - timeseries: (1.25BUY) [input type: string]
                1.) extract the number 1.25
                2.) select the com price for the given timestep
                3.) calculate the price with the factor (1.25)
                    => 1.25 x Buy-Sell-Price(h)
