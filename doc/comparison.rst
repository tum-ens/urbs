...
::
    ...
    if height == 1:
        for i, group in enumerate(CONFIG['SUBPLOTS']):
            plot[i] = totalsum[group].plot.barh(stacked=True, ax=axes[i],
                                                color=colors[group])
...
The following code is still part of the previous ``for``-loop::
        ...
        # thousands
        i = 0
        while totalsum[group].max().max() > 10**(i+4):
            i += 3
        totalsum[group] /= (10**i)
        decimal[group] = (10**i)
...
.. note::
    ...
    ``try`` might not be necessary, but was included to avoid any unexpected exceptions from
    :func:`urbs.to_color`. If no proper color codes are given, the plotting functions will use some default
    colors.
...
:func:`main` primarily handles paths for input and output files.
::
