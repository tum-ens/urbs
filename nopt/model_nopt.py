...
def create_model(data, objective_dict,dt=1, timesteps=None,  dual=True):
    ...
    for key in m.commodity_dict["price"]:
        indexlist.add(tuple(key)[0]) #this steps finds the support frame for each entry (2020, 'Mid', 'Biomass', 'Stock') is the form of each key and [0] is 2020
    m.stf = pyomo.Set(
    ...
    indexlist = set()
    for key in m.commodity_dict["price"]:
        indexlist.add(tuple(key)[1]) #(2020, 'Mid', 'Biomass', 'Stock') is the form of each key and [1] is Mid
    m.sit = pyomo.Set(
        initialize=indexlist,
        doc='Set of sites')
    ...
    m.pro_area_tuples = pyomo.Set(
        within=m.stf * m.sit * m.pro,
        initialize=tuple(m.proc_area_dict.keys()),  #entries that has a process are per cap >= 0
    ...
    m.cap_pro_new = pyomo.Var(
        m.pro_tuples,       #these are indexes of variables
        within=pyomo.NonNegativeReals,
