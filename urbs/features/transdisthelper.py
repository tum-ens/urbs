...
def add_reactive_transmission_lines(microgrid_data_input):
    ...
    reactive_transmission_lines = microgrid_data_input['transmission'][microgrid_data_input['transmission'].index.get_level_values(4) == 'electricity'][~microgrid_data_input['transmission'].loc[:, 'resistance'].isna()]
...
def add_heatpump_flows(microgrid_data_input):
    ### copy transmission lines with resistance to model transmission lines for reactive power flows

    heatpump_flow_lines = microgrid_data_input['transmission'][microgrid_data_input['transmission'].index.get_level_values(4) == 'electricity']
    ...
    ### concat new line data
    microgrid_data_input['transmission'] = pd.concat([microgrid_data_input['transmission'], heatpump_flow_lines], sort=True)
    return microgrid_data_input
    ...
    for process_idx, process in pro_Q.iterrows():
        for ratio_P_idx, ratio_P in ratios_elec.iterrows():
            if process_idx[2] == ratio_P_idx[1]:
                ...
                microgrid_data_input['process_commodity'] = pd.concat([microgrid_data_input['process_commodity'], ratio_Q])
