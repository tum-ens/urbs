...
def add_transmission_ac(m, assumelowq):
    ...
    # Transmission tuples that deliver electricity_hp (electricity for cheaper heatpump tariff)
    ...
    # Variables
    m.cap_tra_new = pyomo.Var(
        m.tra_tuples - m.tra_tuples_hp - m.tra_tuples_reac,
        within=pyomo.NonNegativeReals,
        doc='New transmission capacity (MW)')
    m.cap_tra_decommissioned = pyomo.Var(
        m.tra_decommissionable_tuples - m.tra_tuples_hp - m.tra_tuples_reac,
        within=pyomo.NonNegativeReals,
        doc='Decommissioned transmission capacity (MW)')
    m.cap_tra_unit = pyomo.Var(
        m.tra_block_tuples - m.tra_tuples_hp - m.tra_tuples_reac,
        within=pyomo.NonNegativeIntegers,
        doc='New transmission capacity blocks')
    ...
    m.res_transmission_input_by_capacity = pyomo.Constraint(
        m.tm, m.tra_tuples - m.tra_tuples_ac - m.tra_tuples_hp,
        rule=res_transmission_input_by_capacity_rule,
        doc='transmission input <= total transmission capacity')
    ...
    if not assumelowq:
        m.res_transmission_input_by_apparent_power_diamond_1 = pyomo.Constraint(
            m.tm, m.tra_tuples_ac - m.tra_tuples_hp - m.tra_tuples_reac,
    ...
    m.def_single_ac_cable = pyomo.Constraint(
        m.tra_tuples_ac - m.tra_tuples_hp - m.tra_tuples_reac,
    ...
    m.res_transmission_symmetry = pyomo.Constraint(
        m.tra_tuples_tp - m.tra_tuples_hp,
        rule=res_transmission_symmetry_rule,
        doc='total transmission capacity must be symmetric in both directions')
...
def res_transmission_input_by_apparent_power_diamond_1_rule(m, tm, stf, sin, sout, tra, com):
    if m.mode['hp_elec']:
        hp_el = m.e_tra_in[tm, stf, sin, sout, tra, 'electricity_hp']
    else:
        hp_el = 0
    return (  (m.e_tra_in[tm, stf, sin, sout, tra, 'electricity'] + hp_el)
...
def transmission_cost(m, cost_type):
    ...
    if cost_type == 'Invest':
        cost = sum(m.cap_tra_new[t] *
                   ...
                   for t in m.tra_tuples - m.tra_tuples_hp - m.tra_tuples_reac) - sum(
            m.cap_tra_decommissioned[t] *
            m.transmission_dict['decom-saving'][t] *
            m.transmission_dict['invcost-factor'][t]
            for t in m.tra_decom_cap_dict - m.tra_tuples_hp - m.tra_tuples_reac)
        if m.mode['int']:
            cost -= sum(m.cap_tra_new[t] *
                        m.transmission_dict['inv-cost'][t] *
                        m.transmission_dict['overpay-factor'][t]
                        for t in m.tra_tuples - m.tra_tuples_hp - m.tra_tuples_reac)
