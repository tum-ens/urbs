...
def pyomo_model_prep(data, timesteps):
    ...
    process['support_timeframe'] = (process.index.
                                    get_level_values('support_timeframe'))
    site['support_timeframe'] = (site.index.
                                    get_level_values('support_timeframe'))
    if m.mode['tra']:
...
def get_cluster_data(data, cluster):
    ...
    #cluster_data['process'] = cluster_data['']
    cluster_data['commodity'] = cluster_data['commodity'][
        cluster_data['commodity'].index.get_level_values(1).isin(cluster)]
    cluster_data['process'] = cluster_data['process'][
        cluster_data['process'].index.get_level_values(1).isin(cluster)]
    cluster_data['site'] = cluster_data['site'][
        cluster_data['site'].index.get_level_values(1).isin(cluster)]
    cluster_data['storage'] = cluster_data['storage'][
        cluster_data['storage'].index.get_level_values(1).isin(cluster)]
    #cluster_data['dsm'] = cluster_data['dsm'][
    #    cluster_data['dsm'].index.get_level_values(1).isin(cluster)]
    ...
    try:
        cluster_data['availability'] = cluster_data['availability'][[(sit, pro) for (sit, pro) in cluster_data['availability'].columns if sit in cluster]]
    except:
        pass
    try:
        cluster_data['uhp'] = cluster_data['uhp'][[(sit, pro) for (sit, pro) in cluster_data['uhp'].columns if sit in cluster]]
    except:
        pass

    try:
        cluster_data['uhp'][('ambient','Tamb')]=data['uhp'][('ambient','Tamb')]
    except:
        pass
    return cluster_data
